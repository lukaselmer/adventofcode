...
const input: string = `1224 926 1380 688 845 109 118 88 1275 1306 91 796 102 1361 27 995
...
function calcDifference(row: number[]): number {
  return Math.max(...row) - Math.min(...row);
}

function calcChecksum(): number {
  return sum(matrix.map(calcDifference));
}

function calcDividalbeDevision(row: number[]): number {
  for (const largerCandidate of row) {
    const smallerCandidate = row.find(
      num => largerCandidate !== num && largerCandidate % num === 0
    );

    if (smallerCandidate) return largerCandidate / smallerCandidate;
  }
  throw new Error(`No pairs found in ${row.join(", ")}`);
}

function calcDividableDivisions(): number {
  return sum(matrix.map(calcDividalbeDevision));
}

export function day2() {
  console.log(`Part 1: ${calcChecksum()}`);
  console.log(`Part 2: ${calcDividableDivisions()}`);
}
